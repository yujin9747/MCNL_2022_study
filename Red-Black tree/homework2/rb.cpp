...
class my_map{
    ...
    public :
        ...
        iterator begin(){ // 가장 왼쪽 깊은 곳의 노드에 대한 iterator 반환
            if(this->left == nullptr) return iterator(this);
...
class MapIterator{
    private:
        my_map<T1, T2>* curr;
    public:
        MapIterator(my_map<T1, T2>* p = nullptr)
            :curr(p){}
...
template<typename T1, typename T2>
iterator my_map<T1, T2>::find(T1 key){
    if(this->first.compare(key) == 0) return iterator(this);
    else{
        if(this->left != nullptr) { // 왼쪽 자식 먼저 찾기
            iterator iter = this->left.find(key);
            if(iter != nullptr) return iter;
        }
        else if(this->right != nullptr) { // 왼쪽에서 못 찾은 경우 오른쪽 찾기
            iterator iter = this->right.find(key);
            if(iter != nullptr) return iter;
        }
        else return this->end(); // 왼, 오 모두에서 못 찾은 경우
    }
}
...
*/
void print_map(my_map<string, int> m){
